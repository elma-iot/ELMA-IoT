...
export function safePeripheralPins({chip, inputPins, outputPins, exposedPins, blocked=new Set(), requirement="output", touchPins=[]}) {
  ...
  return inputPins.filter(pin=>exposedPins.has(pin) && !reserved.has(pin)
    && (requirement !== "output" || (output.has(pin) && !(chip === "esp32s2" && pin===46)))
    && (requirement !== "adc" || (adc.has(pin) && !(["esp32s2","esp32s3"].includes(chip) && pin>10))) // Prefer ADC1 while firmware Wi-Fi runs.
