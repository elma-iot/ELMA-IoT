...
struct Preferences{static std::map<std::string,std::string> blobs;static uint32_t revision;static size_t capacity;std::string space;uint32_t getUInt(const char*,uint32_t fallback){return revision?revision:fallback;}size_t putUInt(const char*,uint32_t value){if(!nvsWrite)return 0;revision=value;return 4;}uint8_t getUChar(const char*,uint8_t fallback){return fallback;}bool begin(const char* name,bool){space=name;return true;}void end(){}size_t getBytesLength(const char* key){return blobs[space+":"+key].size();}size_t getBytes(const char* key,void* out,size_t n){auto& blob=blobs[space+":"+key];std::memcpy(out,blob.data(),n);return n;}size_t putBytes(const char* key,const void* data,size_t n){if(!nvsWrite||n>capacity)return 0;blobs[space+":"+key].assign(static_cast<const char*>(data),n);return n;}bool remove(const char* key){blobs.erase(space+":"+key);return true;}bool clear(){for(auto it=blobs.begin();it!=blobs.end();)if(it->first.rfind(space+":",0)==0)it=blobs.erase(it);else ++it;revision=0;return true;}};std::map<std::string,std::string> Preferences::blobs;uint32_t Preferences::revision=0;size_t Preferences::capacity=40000;
#define ELMA_LOGIC_STORAGE_HOST
#include "../src/logic_storage.cpp"
int main(){
 {JsonDocument bulky,loaded;String error;bulky["source"]=99;bulky["mode"]="playing";bulky["graph"]["devices"]=std::string(10000,'x');auto n=bulky["graph"]["nodes"].to<JsonArray>().add<JsonObject>();n["id"]="number";n["type"]="value.number";n["parameters"]["value"]=60;n["position"]["x"]=123;n["ports"]=std::string(10000,'x');n["binding"]=std::string(10000,'x');Preferences::capacity=4096;assert(measureJson(bulky)>20000);assert(saveLogicRecord(bulky.as<JsonVariantConst>(),error));assert(Preferences::blobs["elma-logics:compact"].size()<4096);assert(loadLogicRecord(loaded));assert(loaded["graph"]["nodes"][0]["parameters"]["value"]==60);assert(loaded["graph"]["nodes"][0]["position"]["x"]==123);assert(loaded["graph"]["devices"].isNull());assert(loaded["graph"]["nodes"][0]["ports"].isNull());Preferences::blobs.clear();Preferences::revision=0;Preferences::capacity=40000;latestRevision=0;}
 JsonDocument record,restored;String error;record["source"]=99;record["mode"]="playing";record["graph"]["threshold"]=50;
 sdMounted=true;sd.files["/.elma-logics.json"]="{\"revision\":0,\"source\":99,\"graph\":{\"threshold\":40}}";assert(saveLogicRecord(record.as<JsonVariantConst>(),error));assert(loadLogicRecord(restored));assert(restored["graph"]["threshold"]==50);assert(sd.files.count("/.elma-logics.1.json"));assert(Preferences::blobs.empty());
